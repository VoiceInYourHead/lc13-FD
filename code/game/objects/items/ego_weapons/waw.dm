...
/obj/item/ego_weapon/thirteen/attack(mob/living/M, mob/living/user)
	...
	if(combo >= 13)
		combo = 0
		force = get_modified_attribute_level(user, REFLEXES_STAT)
...
/obj/item/ego_weapon/blind_rage/attack(mob/living/M, mob/living/carbon/human/user)
	...
	var/damage = aoe_damage * (1 + (get_modified_attribute_level(user, REFLEXES_STAT))/100)
...
/obj/item/ego_weapon/mini/heart
	...
	damtype = RED_DAMAGE
	hitsound = 'sound/weapons/bladeslice.ogg'
	attribute_requirements = list(STRENGTH_STAT = 80)
...
/obj/item/ego_weapon/blind_obsession/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	...
	if(charged)
		var/damage = 75
		if(ishuman(thrownby))
			damage *= 1 + (get_modified_attribute_level(thrownby, REFLEXES_STAT))/100
			damage *= force_multiplier
